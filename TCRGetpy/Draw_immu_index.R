...
dir.create(file1)
#names(immdata$data)<-gsub("TRA,TRB.cdrblast","",names(immdata$data))
...
for (i in 1:length(immdata$data)){
  ...
  dlp<-ggplot(cdr_length_table)+geom_bar(aes( x=cdr_length,y=Freq,fill=cdr_length),stat = "identity")+theme(panel.background=element_rect(fill="white",colour="black",size=0.25),
                                                                                                       axis.line=element_line(colour="black",size=0.25),
                                                                                                       axis.title=element_text(size=13,face="plain",color="black"),
                                                                                                       axis.text = element_text(size=12,face="plain",color="black"),
                                                                                                       axis.title.x = element_blank(),
                                                                                                       legend.position="none")+scale_fill_viridis_d()+ylab("CDR3 Freq")+ggtitle(names(immdata$data)[i])
...
#repOverlap(immdata$data) %>% vis()+scale_fill_viridis_c()+xlab("")+ylab("")
...
pw=ggplot(da,aes(x = variable,y = y,fill=Overlap_values))+geom_tile(aes(x = variable,y = y,fill=Overlap_values),
                                                                    color = 'white',size = 0.6,alpha=0.8)+ theme_minimal()+theme(panel.grid=element_blank())+  scale_x_discrete(position = 'top') +
  # 点颜色
  scale_fill_gradientn(colors = mycolor2) +geom_text(aes(label=Overlap_values))+theme(text = element_text(size=16),axis.text.x = element_text(angle = 90,hjust = 1))+xlab("")+ylab("")
...
for(i in 1:length(immdata$data)){
  ...
  pn<-ggplot(genedata,aes(x=Names,y=Clones,fill=Names))+ geom_bar( stat = "identity")+theme(legend.position="none")+xlab("")+theme(panel.background=element_rect(fill="white",colour="black",size=0.25),axis.title=element_text(size=13,face="plain",color="black"),  axis.text = element_text(size=12,face="plain",color="black"),
                                                                                                                                   axis.title.x = element_blank(),legend.position="none" )+scale_fill_viridis_d()+theme(axis.text.x = element_text(angle=90,  hjust=1,   vjust=.5))+ggtitle(names(immdata$data)[i])
...
#grid.arrange(data_geneusage[[1]],data_geneusage[[2]],data_geneusage[[3]])
#genedata<-as.data.frame(geneUsage(immdata$data[[1]]))
...
for (i in 1:length(method_repversity)){
  ...
  #names(data_repversity$data[[i]])<-tem_rep
  
  #data_repversity_plot[[i]]$tem_rep<-repDiversity(immdata$data,tem_rep) %>%vis()+scale_fill_viridis_d()+scale_colour_viridis_d()
  #names(data_repversity_plot[[i]])<-tem_rep
  ggsave(plot=p,filename = paste0(myfile,"/",tem_rep,".pdf"))
  assign(paste0(tem_rep,"_plot"),p)
  
}
#do.call(grid.arrange,data_repversity_plot)
#data_repversity_plot[[1]]$chao1
...
plotdef<-function(data,xnames,yvalue,linelong=c(1),ylabname="Values"){
  p1=ggplot(data = data,aes(x=get(xnames),y=get(yvalue),fill=get(xnames)))+geom_bar( stat = "identity")+
    theme(legend.position="none")+xlab("")+ylab(ylabname)+
    theme(panel.background=element_rect(fill="white",colour="black",size=0.25),axis.title=element_text(size=20,face="plain",color="black"),  axis.text = element_text(size=12,face="plain",color="black"),
          axis.title.x = element_blank(),legend.position="none" )+
    scale_fill_manual(values=mycolor)+theme(axis.text.x = element_text(angle=60,  hjust=0.5,   vjust=.5))+geom_hline(yintercept=linelong,linetype="dashed",alpha=0.3)
